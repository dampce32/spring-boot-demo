...
public class CustomerSpecs {
	...
	public static <T> Specification<T> byAuto(final EntityManager entityManager, final T example) {
		...
		final Class<T> type = (Class<T>) example.getClass();//当前实体类对象类的类型
		return new Specification<T>() {

			@Override
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				List<Predicate> predicates = new ArrayList<>(); //查询条件
				EntityType<T> entity = entityManager.getMetamodel().entity(type);//实体类的EntityType,从EntityType获得实体类的属性
				
				for (Attribute<T, ?> attr : entity.getDeclaredAttributes()) {
					Object attrValue = getValue(example, attr); 
					if (attrValue != null) {
						if (attr.getJavaType() == String.class) { //字符串类型处理
							if (!StringUtils.isEmpty(attrValue)) { //有值时
